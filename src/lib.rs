...
impl NmeaId {
    ...
    pub fn get_pgn(&self) -> Option<PGN> {
        ...
        match self.get_format() {
            NmeaPgnFormat::PDU1 => {
                let pgn = ((self.reserved_data_page() as u32 ) << 16) | ((self.pf() as u32) << 8);
                FromPrimitive::from_u32(pgn)
            }
            NmeaPgnFormat::PDU2 => {
                let pgn = ((self.reserved_data_page() as u32) << 16 | (self.pf() as u32) << 8) | self.ps();
                FromPrimitive::from_u32(pgn)
            }
        }
    }
    ...
    pub fn parse_data(&self, data: &[u8]) -> Result<NmeaData, NmeaError> {
        ...
        match pgn {
            ...
            _ => {
                println!("PGN not implemented: {:?}", pgn);
